...
export interface PendingIssueArrangementItem {
  id: string;
  itemCode: string;
  itemName: string;
  unit: string;
...
export interface ArrangedIssueBinRecord {
  id: string;
  itemCode: string;
  itemName: string;
  store: string;
...
@Component({
  selector: 'app-issue-stock-bin',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule],
  templateUrl: './issue-stock-bin.component.html',
  styleUrl: './issue-stock-bin.component.css'
})
export class IssueStockBinComponent {
  ...
  pendingList = signal<PendingIssueArrangementItem[]>([
    { id: '1', itemCode: 'ITM-MARBLE-01', itemName: 'White Carrara Marble Slab', unit: 'SqFt', issueQty: 120, selectedStore: '', selectedRoom: '', selectedRack: '', selectedBin: '' },
    { id: '2', itemCode: 'ITM-GRANITE-02', itemName: 'Black Galaxy Granite 18mm', unit: 'SqFt', issueQty: 80, selectedStore: '', selectedRoom: '', selectedRack: '', selectedBin: '' },
    { id: '3', itemCode: 'ITM-CEMENT-03', itemName: 'UltraTech Premium Cement', unit: 'Bags', issueQty: 250, selectedStore: '', selectedRoom: '', selectedRack: '', selectedBin: '' }
  ]);

  filteredPendingList = signal<PendingIssueArrangementItem[]>([
    { id: '1', itemCode: 'ITM-MARBLE-01', itemName: 'White Carrara Marble Slab', unit: 'SqFt', issueQty: 120, selectedStore: '', selectedRoom: '', selectedRack: '', selectedBin: '' },
    { id: '2', itemCode: 'ITM-GRANITE-02', itemName: 'Black Galaxy Granite 18mm', unit: 'SqFt', issueQty: 80, selectedStore: '', selectedRoom: '', selectedRack: '', selectedBin: '' },
    { id: '3', itemCode: 'ITM-CEMENT-03', itemName: 'UltraTech Premium Cement', unit: 'Bags', issueQty: 250, selectedStore: '', selectedRoom: '', selectedRack: '', selectedBin: '' }
  ]);
  ...
  doneRecordsList = signal<ArrangedIssueBinRecord[]>([
  ...
  filteredDoneRecordsList = signal<ArrangedIssueBinRecord[]>([
    { id: '1', itemCode: 'ITM-STEEL-05', itemName: 'TMT Steel Rods 12mm', store: 'Main Store A', room: 'Room 101', rack: 'Rack A1', bin: 'Metal Container', arrangedBy: 'Siddharta Dikshit', arrangedDate: '15 May 2026' },
    { id: '2', itemCode: 'ITM-BRICK-04', itemName: 'Red Clay Bricks Grade-A', store: 'Secondary Store B', room: 'Room 102', rack: 'Rack B1', bin: 'Cartoon Box B', arrangedBy: 'Siddharta Dikshit', arrangedDate: '18 May 2026' }
  ]);
  ...
  searchStock() {
    alert(`Searching issue arrangements for ${this.arrangementStatus()} status...`);
  }

  resetForm() {
    this.selectedDepartment.set('');
    this.selectedSlipNo.set('');
  }

  saveItemArrangement(item: PendingIssueArrangementItem) {
    ...
    const newRecord: ArrangedIssueBinRecord = {
    ...
    alert('Issue Stock bin arrangement saved successfully!');
  }

  deleteArrangement(id: string) {
    if (confirm('Are you sure you want to remove this issue bin arrangement?')) {
      const record = this.doneRecordsList().find(r => r.id === id);
      if (record) {
        // Move back to pending list
        const restoredItem: PendingIssueArrangementItem = {
